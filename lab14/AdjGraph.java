...
public class AdjGraph implements Graph {
	...
	public int mstPrim() {
		Integer[] mstList = new Integer[totalNode];
		int u, current;
		...
		for(int i = 0; i < nodeList.size(); i++){
			nodeList1.add(i);
			mstList[i] = Integer.MAX_VALUE;
		}
		mstList[0] = 0;
		
		while(nodeList1.isEmpty() == false){
			u = getVertex(nodeList1, mstList);
			System.out.println("u" +u);
			totalCost += mstList[u];
			nodeList1.remove(nodeList1.indexOf(u));
			LinkedList<Integer> neighbors = adjList.elementAt(u);
					for(int j = 0; j < neighbors.size(); j++){
					  current = neighbors.get(j);
						if(nodeList1.contains(current)){
						if(getWeight(u,current) < mstList[current]){
							mstList[current] = getWeight(u,current);
						}
					}
