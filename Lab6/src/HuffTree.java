...
public class HuffTree<Key, E> {
	...
	private void getCode(BinNode<Integer, String> focus, String History) {
		 if (focus.isLeaf())
		 {
		 codeTable.put(focus.getValue(), History);
		 }
		if(focus.getLeft() != null)
		 {
			getCode(focus.getLeft(), "0" + History);
			getCode(focus.getRight(), "1" + History);
